# Identity and Host-Application Authentication Design

## 1. Purpose and scope

This plugin is a generic IM backend embedded into other projects (mini
programs, mobile apps, websites). Host applications already run their own
...
`(name, uuid)` assigned by the host, carried inside a signed credential
that the plugin verifies statelessly.

This document defines:

- the trust model between the host backend, the client, and the IM
  backend;
...
```text
host backend (trusted)                IM backend (trusted)
...
- Only the host backend and the IM backend know `IM_AUTH_SECRET`.
- The client receives an opaque credential and presents it; it cannot
  forge or alter one.
- The IM backend needs no callback to the host to authenticate a request:
  verification is local HMAC comparison plus a time-window check.
- One IM deployment can serve one or several host applications; all of
  them sign with the same secret. `uuid` values must therefore be unique
  across every integrated host (prefixing, e.g. `shop1:user-42`, is a
...
| --- | --- | --- | --- |
| `uuid` | string, 1–64 chars | yes | Stable, unique identity of the user in the host system. Never reused across users. |
...
format; choose per host-application preference.

### 4.1 Option A: sign in the host backend (recommended)

The host backend signs locally with `IM_AUTH_SECRET`. This adds no network
hop to the host's login flow and works in any language.
...
The host typically mints a credential when its own session is established
(login, token refresh) and returns it to the client alongside its own
session token.

### 4.2 Option B: the IM backend mints on the host's behalf

Hosts that do not want to hold the signing secret (or implement HMAC) call
the server-to-server endpoint during their own login flow:
...
**fetch a fresh credential from the host backend before reconnecting**
when the old one has expired. Hosts should pick a TTL that matches their
own session lifetime (the 24 h default suits most apps).
...
gateway routing reference; the host's `uuid` never appears in delivery
...
2. Switch host backends to sign with the new secret.
...
Credentials minted by the backend (§4.2) sign the current value into the
...
- Revocation invalidates credentials; it does not ban the user. The host
  can mint a fresh credential (carrying the new version) at any time — to
  keep a user out, the host must stop minting for them.
- Host-minted credentials that omit the `token_version` claim (§4.1) are
  unaffected. Deployments that need every client revocable should route
  minting through `/internal/v1/credentials`, or have the host track and
...
- the plugin trusts whatever `(name, uuid)` the host signs — enforcing
  that a host user may only obtain credentials for themselves is the
